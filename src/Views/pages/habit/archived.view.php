...
<body>
    ...
    <?php else : ?>
        ...
        <?php foreach ($habits as $habit) : ?>
            ...
            <form action="/archived/<?= $habit['habit_id']; ?>" method="POST"
                onsubmit="return confirm('Are you sure you want to restore soft-deleted habit Nr.<?= $habit['habit_id']; ?>?');">
            ...
            ?>
            <form action="/archived/<?= $habit['habit_id']; ?>" method="POST"
                onsubmit="return confirm('Are you sure you want to force-delete habit Nr.<?= $habit['habit_id']; ?>?');">
                <input type="hidden" name="__spoof_method" value="DELETE">

                <div>
                    <button type="submit">🗑️ Completely delete</button>
                </div>
            </form>
